...
const BuddyChatMessages = ({ messages, buddyState }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="min-h-full flex flex-col justify-end px-4 py-3 gap-2.5">
        {/* Empty state */}
        {messages.length === 0 && !isLoading && (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-card/50 backdrop-blur-md border border-border/20 rounded-2xl px-5 py-4 max-w-[80%]">
          ...
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
            <div
              className={`max-w-[85%] px-3.5 py-2.5 text-[14px] leading-relaxed shadow-lg ${
                msg.role === "assistant"
                  ? "bg-card/80 backdrop-blur-md border border-primary/15 text-foreground rounded-2xl rounded-tl-sm"
                  : "bg-primary/25 backdrop-blur-md border border-primary/20 text-foreground rounded-2xl rounded-tr-sm"
              }`}
            >
              {msg.role === "assistant" && (
        ...
        {isLoading && messages[messages.length - 1]?.role === "user" && (
          <div className="flex justify-start animate-fade-in">
            <div className="bg-card/80 backdrop-blur-md border border-primary/15 rounded-2xl rounded-tl-sm px-4 py-3 shadow-lg">
...
export default BuddyChatMessages;
